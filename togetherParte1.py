...
log_file = os.path.abspath('together1Continue.log')
logging.basicConfig(
    filename=log_file,
    level=logging.INFO,
...
def process_csv(file_path, output_file, batch_size=50, delay=1):
    ...
    df.reset_index(drop=True, inplace=True)  # Reset index to ensure it's integer
    total_rows = len(df)
...
if __name__ == "__main__":
    ...
    input_csv = os.path.join(base_dir, "parte_1_continue.csv") 
    output_dir = os.path.join(base_dir, "together1Continue")
    
    # Asegurar que el directorio de salida exista
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
        
    output_csv = os.path.join(output_dir, "together1Continue.csv")
